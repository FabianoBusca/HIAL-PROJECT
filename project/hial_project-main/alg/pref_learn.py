...
def learn_weights(
    ...
    *,
    ...
    feat_dim = int(traj_set.features_matrix.shape[1])
    params: Dict[str, Any] = {"weights": util_funs.get_random_normalized_vector(feat_dim)}
    user_model = SoftmaxUser(params)
    belief = SamplingBasedBelief(user_model, [], params)

    print("Initial estimated weights:", np.asarray(belief.mean["weights"]).round(3))

    ##############################################################################

    query: PreferenceQuery = PreferenceQuery(traj_set[:2])

    collected_prefs: List[Preference] = []

    for q in range(int(num_queries)):
        queries, objective_values = optimizer.optimize(acquisition_function, belief, query)
        optimized_query: PreferenceQuery = queries[0]

        response = optimized_query.visualize(delay=0.2)

        pref = Preference(optimized_query, response)
        collected_prefs.append(pref)
        belief.update(pref)

        w_mean = np.asarray(belief.mean["weights"], dtype=np.float32)
        print(
            f"[Query {q + 1}/{num_queries}] "
            f"objective={objective_values[0]:.6f}  "
            f"w_mean={np.round(w_mean, 3)}"
        )
    ...
    return np.asarray(belief.mean["weights"], dtype=np.float32)
...
def main() -> None:
    ...
    try:
        ...
        print(f"\n[WARNING] Preference learning interrupted: {e}")
        print("Attempting to save partially-learned weights from last belief state...")
        raise
    finally:
        env.close()
