...
def feature_function(traj_pairs: List[Tuple[Dict[str, np.ndarray], np.ndarray]]) -> np.ndarray:
    ...
    states = [sa[0] for sa in traj_pairs]
    goal = np.asarray(states[0]["desired_goal"], dtype=np.float32)

    dists: List[float] = []
    first_lift_t: Optional[int] = None

    for t, s in enumerate(states):
        obj_pos = np.asarray(s["observation"], dtype=np.float32)[7:10]
        d = float(np.linalg.norm(obj_pos - goal))
        dists.append(d)

        if first_lift_t is None and float(obj_pos[2]) > 0.08:
            first_lift_t = t

    final_dist = float(dists[-1])
    min_dist = float(np.min(dists))
    T = max(1, len(states))
    tfl = -1.0 if first_lift_t is None else -(float(first_lift_t) / float(T))

    return np.array([-final_dist, -min_dist, tfl], dtype=np.float32)
    ...
    *,
    ...
    for t in range(T):
        ...
        if terminated or truncated:
            print(f"Success: {info.get('is_success')}")
            break
...
def main() -> None:
    ...
    for i, demo in enumerate(demos):
        ...
        traj_pairs, frames = rollout(env, action_traj, options=options)

        clip_path = clips_dir / f"expert_{i:02d}.mp4"
        with imageio.get_writer(str(clip_path), **writer_kwargs) as writer:
            for f in frames:
                writer.append_data(np.asarray(f, dtype=np.uint8))

        feats = feature_function(traj_pairs)
        saved_records.append(TrajectoryRecord(clip_path=str(clip_path), features=feats))
        print(f"  wrote {clip_path} (T={len(traj_pairs)})")
    ...
    for i in range(10):
        ...
        clip_path = clips_dir / f"random_{i:02d}.mp4"
        with imageio.get_writer(str(clip_path), **writer_kwargs) as writer:
            for f in frames:
                writer.append_data(np.asarray(f, dtype=np.uint8))
    ...
    env.close()
    print("\nAll clips generated.")
    print(f"Total trajectories prepared for APReL: {len(saved_records)}")
